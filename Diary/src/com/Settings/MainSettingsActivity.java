...
public class MainSettingsActivity extends PreferenceFragment {
	...
	public void reset(){
		...
		Intent newEntry = new Intent(this.getActivity().getApplicationContext(), NewEntry.class);
		newEntry.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP
		...
		themePref.setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
			...
			public boolean onPreferenceChange(Preference preference, Object newValue) {
				...
				delayedActionWeak.get().postDelayed(new Runnable() {
					@Override
					public void run() {
						sp.edit().putBoolean("THEMECHANGE", true).apply();
						parent.recreate();
					}
				}, 100);
				return true;
    ...
    {   
        ...
        public boolean onPreferenceChange(Preference preference, Object newValue) 
        {
        	if (sp.getBoolean("Password", false) == false){
        		//Toast.makeText(c.getApplicationContext(), "File created",
    			//Toast.LENGTH_LONG).show();
